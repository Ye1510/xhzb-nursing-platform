...
import com.xhzb.nursing.domain.vo.FloorTreeVo;
...
public class FloorServiceImpl extends ServiceImpl<FloorMapper, Floor> implements IFloorService
{
    ...
    /**
     * 根据床位状态查询楼层→房间→床位树形结构
     */
    @Override
    public List<FloorTreeVo> getFloorTreeByBedStatus(Integer status) {
        List<FloorTreeVo> treeList = floorMapper.selectFloorTreeByBedStatus(status);
        if (treeList == null) {
            return List.of();
        }
        return treeList;
